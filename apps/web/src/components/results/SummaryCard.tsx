...
export function SummaryCard({ item }: SummaryCardProps) {
    ...
    return (
        <Card className="group w-full transition-all hover:scale-[1.01] hover:shadow-lg flex flex-col">
            <CardHeader>
                <CardDescription className="flex h-fit w-fit rounded-xl bg-blue-100 p-3">
                ...
                {item.variant === "figures" && item.files && (
                    <div className="flex flex-wrap gap-2 mt-auto pt-2">
                        {item.files.map((f) => (
                            <span
                                key={f.name}
                                className="text-xs bg-gray-100 text-gray-600 rounded-md px-2 py-1 font-mono"
                            >
                                {f.name}
                            </span>
                        ))}
                        {item.extraFiles && item.extraFiles > 0 && (
                            <span className="text-xs bg-blue-100 text-blue-600 rounded-md px-2 py-1">
